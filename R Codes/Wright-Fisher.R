...
g1_old <- t(t(read.table("./Data/output/cont2a_test4.mat"))) 
g2_old <- t(t(read.table("./Data/output/cont2b_test4.mat") ))

# Gamma_2 for portkey Barker's
g1_new <- t(t(read.table("./Data/output/cont2a_new4.mat") ))
g2_new <- t(t(read.table("./Data/output/cont2b_new4.mat") ))


pdf("./figures/WF_loops12.pdf", height = 5, width = 7)
par(mfrow = c(1,2))
...
# dev.off()
# 
# pdf("./figures/WF_loops2.pdf", height = 5, width = 5)
...
ga_old <- read.table("./Data/output/gama_test4.mat")
ga_old <- t(t(ga_old))

ga_new <- read.table("./Data/output/gama_new4.mat")
ga_new <- t(t(ga_new))

pdf("./figures/WF_gamma12.pdf", height = 5, width = 7)
par(mfrow=c(1,2))
plot(density(ga_old[,1]), xlab = expression(gamma[1]), main = "", lty = 2)
lines(density(ga_new[,1]), col = "red")
legend("topleft", legend = c("Barker's", "Portkey Barker's"), 
       lty = c(2,1), col = c("black", "red"),
       cex = 0.55)
# dev.off()
# 
# pdf("./figures/WF_gamma2.pdf", height = 5, width = 5)
plot(density(ga_old[,2]), xlab = expression(gamma[2]), main = "", lty = 2, ylim = range(density(ga_new[,2])$y))
lines(density(ga_new[,2]), col = "red")
legend("topleft", legend = c("Barker's", "Portkey Barker's"), lty = c(2,1),
       col = c("black", "red"), cex = 0.54)
dev.off()

pdf("./figures/WF_acf12.pdf", height = 5, width = 7)
par(mfrow = c(1,2))
...
# dev.off()
# 
# pdf("./figures/WF_acf2.pdf", height = 5, width = 5)
# par(mfrow = c(1,1))
